    ...
    try:
        ...
        if success:
            logging_utility.info(
                "Account deleted successfully for user ID: %s", user_id
            )
            return jsonify({"message": "Account deleted successfully."}), 200
        else:
            logging_utility.error(
                "An error occurred while deleting the account for user ID: %s", user_id
            )
            return (
                jsonify({"error": "An error occurred while deleting the account."}),
                500,
            )
