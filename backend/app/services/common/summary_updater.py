...
if not gpt3_turbo_service_drone_id:
    raise ValueError(
        "OpenAI API key, GPT-4 Turbo Assistant ID, or GPT-3 Turbo Assistant ID is not defined. Please check your environment variables."
    )
...
class SummaryUpdaterService:
    ...
    def process_thread_with_drone(self, thread_id, user_id):
        ...
        with app.app_context():
            session = self.db_service.fetch_session_by_thread_id_and_user(
                thread_id, user_id
            )
            if session:
                messages = session.get_messages()
                if messages:
                    most_recent_message = messages[
                        -1
                    ]  # Assuming the last message is the most recent
    ...
    def generate_summary(self, entities, pos_tags, dependency_parse, keywords):
        ...
        if entities:
            entity_summary = ", ".join(
                [f"{entity[0]} ({entity[1]})" for entity in entities]
            )
        ...
        if dependency_parse:
            simplified_parse = [
                f"{token[0]} ({token[1]})" for token in dependency_parse
            ]
        ...
        with app.app_context():
            ...
            if session:
                ...
                if messages:
                    ...
                    pos_tags = self.context_extractor.tag_parts_of_speech(
                        message_content
                    )
                    dependency_parse = self.context_extractor.parse_dependency(
                        message_content
                    )
                    extracted_keywords = self.context_extractor.extract_keywords(
                        message_content
                    )

                    # Generate a human-readable summary from the extracted information
                    summary = self.generate_summary(
                        entities, pos_tags, dependency_parse, extracted_keywords
                    )
                ...
                print(
                    f"No session found for thread ID: {thread_id} and user ID: {user_id}"
                )
...
if __name__ == "__main__":
    # Example usage
    thread_id = (
        "specific_thread_id"  # Replace with the actual thread ID you want to update
    )
