...
class GoogleGeocodingService:
    ...
    def get_last_seen_user_location(self, faux_identity, limit):
        ...
        last_seen_user_location = self.user_location_service.get_user_locations(
            faux_identity=faux_identity, limit=limit
        )
    ...
    def get_elevation(self, locations):
        params = {
            "locations": "|".join(
                [f"{location['lat']},{location['lng']}" for location in locations]
            ),
            "key": self.api_key,
        }
        ...
        if elevation_data["status"] == "OK":
            url = self.compose_url(
                "elevation", lat=locations[0]["lat"], lng=locations[0]["lng"]
            )
    ...
    def get_current_location_by_gps(self, faux_identity, limit):
        ...
        user_gps_location = self.get_last_seen_user_location(
            faux_identity=faux_identity, limit=limit
        )
