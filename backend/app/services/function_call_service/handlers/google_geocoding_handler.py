from backend.app.services.api_service_external.api_proprietary_access.api_google.google_geocoding_service import \
...
class GoogleGeocodingHandler:
    ...
    def handle_get_current_location_by_gps(self, arguments):
        ...
        if faux_identity:
            try:
                location = self.google_geocoding_service.get_current_location_by_gps(
                    faux_identity, limit
                )
                ...
                self.logging_utility.error(
                    f"Error retrieving current location by GPS: {str(e)}"
                )
    ...
    def handle_get_places_near_by_gps(self, arguments):
        ...
        if faux_identity and place:
            try:
                data = self.google_geocoding_service.get_places_near_by_gps(
                    faux_identity, limit, place
                )
                ...
                self.logging_utility.error(
                    f"Error retrieving places nearby by GPS: {str(e)}"
                )
