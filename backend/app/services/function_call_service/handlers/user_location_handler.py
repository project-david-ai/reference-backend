...
class UserLocationHandler:
    ...
    def handle_get_user_locations(self, arguments):
        ...
        if faux_identity:
            user_locations = self.user_location_service.get_user_locations(faux_identity, limit)

            if user_locations:
                response = "User Locations:\n\n"
                for location in user_locations:
                    response += f"ID: {location.get('id', '')}\n"
                    response += f"Permission Status: {location.get('permission_status', '')}\n"
                ...
                self.logging_utility.warning('Failed to retrieve user locations for faux identity: %s', faux_identity)
